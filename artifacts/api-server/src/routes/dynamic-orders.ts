...
router.post("/orders", async (req, res): Promise<void> => {
  ...
  try {
    ...
    for (const selected of data.services) {
      const quantity = Number(selected.quantity); if (!Number.isInteger(quantity) || quantity <= 0) { res.status(400).json({ error: "Service quantity must be a positive whole number." }); return; }
      ...
      if (code.startsWith("package:")) {
        ...
        const urgent = Boolean((selected as any).urgent); if (urgent && !urgentPossible) { res.status(400).json({ error: `${pack.name} cannot be processed urgently because one of its services does not allow urgent processing.` }); return; }
        ...
        const urgent = Boolean((selected as any).urgent); if (urgent && !service.urgent_allowed) { res.status(400).json({ error: `${service.name} does not allow urgent processing.` }); return; }
        ...
        totalAmount += unitPrice * quantity; maxDeliveryDays = Math.max(maxDeliveryDays, urgent ? Number(service.urgent_delivery_days) : Number(service.normal_delivery_days));
