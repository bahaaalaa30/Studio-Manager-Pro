...
export default function Photography() {
  ...
  const handleStatusChange = (id: number, status: "IN_PHOTOGRAPHY" | "WAITING_EDITING") => {
    updateStatus.mutate({ id, data: { status } }, {
      onSuccess: () => queryClient.invalidateQueries({ queryKey: getListOrdersQueryKey() }),
    });
  };

  const waitingOrders   = orders.filter(o => o.status === "WAITING_PHOTOGRAPHY" || o.status === "NEW");
  const inProgressOrders = orders.filter(o => o.status === "IN_PHOTOGRAPHY");
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto w-full flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Manage walk-ins and active shoots.</p>
      </div>
              ...
              <OrderCard
                ...
                actions={
                  <Button size="sm" className="w-full"
                    onClick={() => handleStatusChange(order.id, "IN_PHOTOGRAPHY")}
              ...
              <OrderCard
                ...
                actions={
                  <Button size="sm" className="w-full bg-green-600 hover:bg-green-700 text-white"
                    onClick={() => handleStatusChange(order.id, "WAITING_EDITING")}
                    disabled={updateStatus.isPending}
                  >
                    Finish Shoot
                  </Button>
