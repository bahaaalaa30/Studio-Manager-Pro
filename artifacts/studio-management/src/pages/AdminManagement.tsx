...
function statusClass(value: unknown) { return String(value ?? "").toUpperCase() === "ACTIVE" ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "bg-muted text-muted-foreground"; }
...
function validateBranch(form: Record<string, string>): Record<string, string> {
  const errors: Record<string, string> = {};
  ...
  else if (!/^[A-Za-z0-9_-]+$/.test(code)) errors.code = "Use letters, numbers, hyphens or underscores only.";
  if (!form.status) errors.status = "Status is required.";
  if (address && !/^[A-Za-z0-9\u0600-\u06FF]+(?:[ ,./#-][A-Za-z0-9\u0600-\u06FF]+)*$/.test(address)) errors.address = "Use letters, numbers, spaces and common address separators only.";
  if (phone && !/^\d{7,15}$/.test(phone)) errors.phone = "Use numbers only, 7 to 15 digits.";
  return errors;
...
export default function AdminManagement({ resource }: { resource: Resource }) {
  ...
  const [roles, setRoles] = useState<Option[]>([]); const [branches, setBranches] = useState<Option[]>([]); const [managers, setManagers] = useState<Option[]>([]); const [showPassword, setShowPassword] = useState(false);
  ...
  const loadUserOptions = async () => {
    try {
      const [rolesResponse, branchesResponse] = await Promise.all([fetch("/api/admin/roles", { headers: { Accept: "application/json" } }), fetch("/api/admin/branches", { headers: { Accept: "application/json" } })]);
      const rolesData = await readApiResponse(rolesResponse); const branchesData = await readApiResponse(branchesResponse);
      if (rolesResponse.ok) setRoles(Array.isArray(rolesData) ? rolesData.filter((r): r is Option => typeof r === "object" && r !== null && typeof (r as Option).id === "number" && isActive((r as Option).status)) : []);
      if (branchesResponse.ok) setBranches(Array.isArray(branchesData) ? branchesData.filter((b): b is Option => typeof b === "object" && b !== null && typeof (b as Option).id === "number" && isActive((b as Option).status)) : []);
    } catch { setRoles([]); setBranches([]); }
  };

  const loadManagerOptions = async () => {
    try { const response = await fetch("/api/admin/options/users", { headers: { Accept: "application/json" } }); const data = await readApiResponse(response); if (!response.ok) throw new Error(apiError(data, "Failed to load managers.")); setManagers(Array.isArray(data) ? data.filter((u): u is Option => typeof u === "object" && u !== null && typeof (u as Option).id === "number") : []); }
    catch { setManagers([]); }
  };

  const openCreate = () => {
    const next = resource === "users" || resource === "branches" ? { status: "ACTIVE" } : {};
    setEditing({}); setForm(next); setInitialForm(next); setShowPassword(false); setError("");
    if (resource === "users") void loadUserOptions();
    if (resource === "branches") void loadManagerOptions();
  };

  const openEdit = (row: RecordData) => {
    const next: Record<string, string> = Object.fromEntries(config.fields.map((f) => [f, row[f] === null || row[f] === undefined ? "" : String(row[f])]));
    setEditing(row); setForm(next); setInitialForm(next); setShowPassword(false); setError("");
    if (resource === "users") void loadUserOptions();
    if (resource === "branches") void loadManagerOptions();
  };

  const userErrors = resource === "users" && editing ? validateUser(form, !editing.id) : {};
  const branchErrors = resource === "branches" && editing ? validateBranch(form) : {};
  const validationErrors = resource === "users" ? userErrors : resource === "branches" ? branchErrors : {};
  const dirty = editing ? formSignature(form) !== formSignature(initialForm) : false;
  const canSave = !!editing && dirty && !saving && (resource === "users" || resource === "branches" ? Object.keys(validationErrors).length === 0 : true);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (resource === "users") {
        for (const field of ["name", "username", "role_id", "branch_id", "status", "address", "phone", "password"]) if (form[field] !== undefined && (field !== "password" || form[field] !== "")) body[field] = ["role_id", "branch_id"].includes(field) ? Number(form[field]) : field === "status" ? form[field].toUpperCase() : form[field];
      } else if (resource === "branches") {
        for (const field of ["name", "code", "address", "phone", "manager_user_id", "status"]) if (form[field] !== undefined) body[field] = field === "manager_user_id" ? (form[field] === "" ? null : Number(form[field])) : field === "status" ? form[field].toUpperCase() : form[field];
      } else {
        for (const field of config.fields) if (form[field] !== undefined && form[field] !== "") body[field] = inputType(field) === "number" ? Number(form[field]) : form[field];
      }
      ...
      const data = await readApiResponse(response); if (!response.ok) throw new Error(apiError(data, "Save failed."));
  ...
  const columns = useMemo(() => {
    if (resource === "users") return ["name", "username", "role_name", "branch_name", "status"];
    if (resource === "branches") return ["name", "code", "manager_name", "phone", "status"];
    return config.fields.slice(0, resource === "permissions" ? 5 : 6);
  }, [config.fields, resource]);
  const columnLabel = (column: string) => column === "role_name" ? "Role" : column === "branch_name" ? "Branch" : column === "manager_name" ? "Manager" : config.labels[column] ?? column;
  ...
  const renderSelect = (label: string, value: string, options: Option[], onChange: (value: string) => void, required = true, errorText?: string, placeholder = "Choose") => <div className="space-y-1.5"><Label>{label} {required && <span className="text-destructive">*</span>}</Label><select value={value} onChange={(e) => onChange(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-ring" aria-invalid={!!errorText}><option value="">{placeholder}</option>{options.map((item) => <option key={item.id} value={item.id}>{item.name}{item.username ? ` (${item.username})` : ""}</option>)}</select>{errorText && <p className="text-xs text-destructive">{errorText}</p>}</div>;
    ...
    {editing && <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4"><Card className="w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl"><CardHeader className="sticky top-0 z-20 bg-card border-b flex flex-row items-center justify-between"><div><CardTitle>{editing.id ? "Edit" : "Add"} {config.title.replace(/s$/, "")}</CardTitle><p className="text-xs text-muted-foreground mt-1">{resource === "users" ? "Create a secure staff account and assign its access scope." : resource === "branches" ? "Keep branch master data consistent and ready for user assignment." : "Keep the master data consistent across the studio."}</p></div><Button variant="ghost" size="icon" onClick={() => setEditing(null)}><X className="w-4 h-4" /></Button></CardHeader><CardContent className="p-5 space-y-5">
      {resource === "users" ? <>
        ...
        <div className="border-t pt-5"><div className="text-sm font-semibold mb-3">Access & assignment</div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {renderSelect("Role", form.role_id ?? "", roles, (value) => setForm((p) => ({ ...p, role_id: value })), true, userErrors.role_id, "Choose a role")}
          {renderSelect("Assign Branch", form.branch_id ?? "", branches, (value) => setForm((p) => ({ ...p, branch_id: value })), true, userErrors.branch_id, branches.length ? "Choose a branch" : "No active branches available")}
        ...
        {(!roles.length || !branches.length) && <div className="flex gap-2 rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2.5 text-xs text-amber-700 dark:text-amber-400"><AlertCircle className="w-4 h-4 shrink-0 mt-0.5" /><span>{!roles.length && !branches.length ? "Roles and branches must be configured before creating a user." : !roles.length ? "No active roles are available." : "No active branches are available."}</span></div>}
      </> : resource === "branches" ? <>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5"><Label>Branch Name <span className="text-destructive">*</span></Label><Input value={form.name ?? ""} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. Main Branch" aria-invalid={!!branchErrors.name} />{branchErrors.name && <p className="text-xs text-destructive">{branchErrors.name}</p>}</div>
          <div className="space-y-1.5"><Label>Code <span className="text-destructive">*</span></Label><Input value={form.code ?? ""} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value.toUpperCase() }))} placeholder="e.g. MAIN01" aria-invalid={!!branchErrors.code} />{branchErrors.code && <p className="text-xs text-destructive">{branchErrors.code}</p>}<p className="text-[11px] text-muted-foreground">Unique code. Letters, numbers, hyphens and underscores.</p></div>
          <div className="space-y-1.5 sm:col-span-2"><Label>Address <span className="text-[11px] text-muted-foreground font-normal">(optional)</span></Label><Input value={form.address ?? ""} onChange={(e) => setForm((p) => ({ ...p, address: e.target.value }))} placeholder="Optional branch address" aria-invalid={!!branchErrors.address} />{branchErrors.address && <p className="text-xs text-destructive">{branchErrors.address}</p>}</div>
          <div className="space-y-1.5"><Label>Phone <span className="text-[11px] text-muted-foreground font-normal">(optional)</span></Label><Input type="tel" inputMode="numeric" value={form.phone ?? ""} onChange={(e) => setForm((p) => ({ ...p, phone: e.target.value }))} placeholder="Optional branch phone" aria-invalid={!!branchErrors.phone} />{branchErrors.phone && <p className="text-xs text-destructive">{branchErrors.phone}</p>}</div>
          {renderSelect("Manager", form.manager_user_id ?? "", managers, (value) => setForm((p) => ({ ...p, manager_user_id: value })), false, undefined, "No manager")}
          <div className="space-y-1.5"><Label>Status <span className="text-destructive">*</span></Label><select value={form.status ?? ""} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value }))} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-ring" aria-invalid={!!branchErrors.status}><option value="">Choose status</option><option value="ACTIVE">Active</option><option value="INACTIVE">Not Active</option></select>{branchErrors.status && <p className="text-xs text-destructive">{branchErrors.status}</p>}</div>
        </div>
        <div className="flex gap-2 rounded-lg border border-blue-500/20 bg-blue-500/5 px-3 py-2.5 text-xs text-muted-foreground"><AlertCircle className="w-4 h-4 shrink-0 mt-0.5" /><span>Manager is optional. Only active users can be assigned as branch managers.</span></div>
      ...
      <div className="flex justify-between items-center gap-3 pt-4 border-t"><p className="text-[11px] text-muted-foreground">{resource === "users" || resource === "branches" ? (editing.id ? (dirty ? "Unsaved changes" : "No changes to save") : "Fields marked * are required.") : dirty ? "Unsaved changes" : "No changes to save"}</p><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setEditing(null)}>Cancel</Button><Button onClick={() => void save()} disabled={!canSave} className="gap-2"><Save className="w-4 h-4" />{saving ? "Saving…" : "Save changes"}</Button></div></div>
