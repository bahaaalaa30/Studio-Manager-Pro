...
export default function Admin() {
  ...
  const revenueTrendData = useMemo(() => !stats ? [] : isMultiDay
    ? stats.dailyRevenue.map((d) => ({ label: fmtDate(d.date), revenue: d.revenue, orders: d.orders }))
    : stats.hourlyDistribution.map((h) => ({ label: fmtHour(h.hour), orders: h.count })), [stats, isMultiDay]);

  const servicePieData = useMemo(() => (stats?.serviceBreakdown ?? [])
    .filter((s) => s.serviceType !== "urgent_fee")
    .map((s) => ({ name: SERVICE_LABELS[s.serviceType] ?? s.serviceType, value: s.revenue })), [stats]);
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      return [order.orderNumber, order.customerName, order.customerMobile, order.customerType, order.status, order.paymentMethod, order.notes, serviceText]
        .filter(Boolean).join(" ").toLowerCase().includes(q);
    });
  }, [orders, search, statusFilter, paymentFilter]);

  const exportOrders = () => {
    const headers = ["ID", "Order Number", "Customer Name", "Mobile", "Customer Type", "Services", "Total", "Paid", "Remaining", "Payment Method", "Expected Delivery", "Status", "Notes", "Created At", "Updated At"];
    const rows = filteredOrders.map((order) => [
      ...
      order.customerType,
      order.services.map((s) => `${s.quantity ?? 0}x ${s.serviceName ?? s.serviceType ?? ""} @ ${s.unitPrice ?? 0} = ${s.totalPrice ?? 0}`).join(" | "),
      order.totalAmount,
      order.paidAmount,
      order.remainingAmount,
      order.paymentMethod,
      order.expectedDeliveryTime ? fmtDateTime(order.expectedDeliveryTime) : "",
      order.status,
      order.notes ?? "",
    ...
    anchor.href = url;
    anchor.download = `smp-orders-${from}-to-${to}.csv`;
    anchor.click();
  ...
  return <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto w-full space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2"><LayoutDashboard className="w-5 h-5 sm:w-6 sm:h-6 text-primary" />Orders Analytics</h2>
      ...
      <div className="flex gap-2">
        <Button variant="outline" size="sm" onClick={exportOrders} disabled={!filteredOrders.length} className="gap-2"><Download className="w-3.5 h-3.5" />Export CSV</Button>
    ...
    <Card>
      <CardHeader className="pb-3"><div className="flex flex-col xl:flex-row xl:items-center justify-between gap-3"><CardTitle className="text-sm font-semibold">All Orders</CardTitle><div className="flex flex-col sm:flex-row gap-2"><div className="relative"><Search className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-muted-foreground" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search order, customer, mobile, service..." className="h-8 w-full sm:w-72 rounded-md border bg-background pl-8 pr-3 text-xs outline-none focus:ring-1 focus:ring-primary" /></div><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-8 rounded-md border bg-background px-2 text-xs"><option value="all">All statuses</option>{statusOptions.map((status) => <option key={status} value={status}>{status}</option>)}</select><select value={paymentFilter} onChange={(e) => setPaymentFilter(e.target.value)} className="h-8 rounded-md border bg-background px-2 text-xs"><option value="all">All payments</option>{paymentOptions.map((method) => <option key={method} value={method}>{method}</option>)}</select></div></div></CardHeader>
      <CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-xs"><thead className="bg-muted/50 border-y"><tr>{["ID", "Order", "Customer", "Mobile", "Type", "Services", "Total", "Paid", "Remaining", "Payment", "Expected Delivery", "Status", "Notes", "Created", "Updated"].map((head) => <th key={head} className="px-3 py-2 text-left font-semibold whitespace-nowrap">{head}</th>)}</tr></thead><tbody>{statsLoading ? <tr><td colSpan={15} className="p-8 text-center text-muted-foreground">Loading orders…</td></tr> : filteredOrders.length === 0 ? <tr><td colSpan={15} className="p-8 text-center text-muted-foreground">No orders match the current filters.</td></tr> : filteredOrders.map((order) => <tr key={order.id} className="border-b hover:bg-muted/30 align-top"><td className="px-3 py-2 font-mono">{order.id}</td><td className="px-3 py-2 font-medium whitespace-nowrap">{order.orderNumber}</td><td className="px-3 py-2 max-w-[180px] break-words">{order.customerName || "Walk-in"}</td><td className="px-3 py-2 whitespace-nowrap">{order.customerMobile}</td><td className="px-3 py-2 whitespace-nowrap">{order.customerType}</td><td className="px-3 py-2 min-w-[240px]"><div className="space-y-1">{order.services.map((service, index) => <div key={`${order.id}-${index}`}><span className="font-medium">{service.quantity ?? 0}× {service.serviceName ?? SERVICE_LABELS[service.serviceType ?? ""] ?? service.serviceType}</span><span className="text-muted-foreground"> · {formatCurrency(Number(service.unitPrice ?? 0))} × {service.quantity ?? 0} = {formatCurrency(Number(service.totalPrice ?? 0))}</span>{service.urgent && <span className="ml-1 text-destructive">(urgent)</span>}</div>)}</div></td><td className="px-3 py-2 font-mono whitespace-nowrap">{formatCurrency(order.totalAmount)}</td><td className="px-3 py-2 font-mono whitespace-nowrap text-emerald-600">{formatCurrency(order.paidAmount)}</td><td className={`px-3 py-2 font-mono whitespace-nowrap ${order.remainingAmount > 0 ? "text-destructive" : ""}`}>{formatCurrency(order.remainingAmount)}</td><td className="px-3 py-2 whitespace-nowrap">{order.paymentMethod}</td><td className="px-3 py-2 whitespace-nowrap">{order.expectedDeliveryTime ? fmtDateTime(order.expectedDeliveryTime) : "—"}</td><td className="px-3 py-2 whitespace-nowrap"><span className="rounded-full bg-muted px-2 py-1 font-medium">{order.status}</span></td><td className="px-3 py-2 min-w-[180px] max-w-[260px] break-words">{order.notes || "—"}</td><td className="px-3 py-2 whitespace-nowrap">{fmtDateTime(order.createdAt)}</td><td className="px-3 py-2 whitespace-nowrap">{fmtDateTime(order.updatedAt)}</td></tr>)}</tbody></table></div><div className="border-t px-3 py-2 text-xs text-muted-foreground">Showing {filteredOrders.length} of {orders.length} orders for {fmtDate(from)} to {fmtDate(to)}.</div></CardContent>
    </Card>
  </div>;
}
