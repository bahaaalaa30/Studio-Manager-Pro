...
const settingItems: SettingItem[] = [
  ...
  { key: "branches", title: "Branches", description: "Manage studio branches, contact details and managers.", href: "/admin/branches", permission: "branches.view", icon: Building2 },
  { key: "roles", title: "Roles", description: "Create roles and control the permissions assigned to each role.", href: "/admin/roles", permission: "roles.view", icon: ShieldCheck },
  { key: "permissions", title: "Permissions", description: "Manage the permission catalog used by roles and authorization.", href: "/admin/permissions", permission: "permissions.view", icon: KeyRound },
  { key: "services", title: "Services", description: "Manage services available when creating studio orders.", href: "/admin/services", permission: "services.view", icon: BriefcaseBusiness },
  { key: "packages", title: "Packages", description: "Manage service packages, pricing and availability.", href: "/admin/packages", permission: "packages.view", icon: Boxes },
...
export default function SettingsHome() {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me", { headers: { Accept: "application/json" }, credentials: "same-origin", cache: "no-store" })
      ...
      .then((raw) => {
        if (!cancelled) {
          setPermissions(buildPermissionSet(raw));
          setLoaded(true);
        }
      })
      .catch(() => {
    ...
    <div className="min-h-full bg-background p-5 md:p-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8">
          ...
          <h1 className="mt-2 text-2xl md:text-3xl font-bold tracking-tight text-foreground">Settings</h1>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">Manage your studio master data and access controls. You only see areas your role is allowed to access.</p>
        </div>

        {!loaded ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-36 rounded-xl border bg-card animate-pulse" />)}
          </div>
            ...
            {visibleItems.map((item) => {
              ...
              return (
                <Link key={item.key} href={item.href} className="group block">
                  <Card className="h-full border-border/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5">
          ...
          <Card>
            <CardContent className="flex min-h-48 items-center justify-center p-8 text-center">
              <div>
                ...
                <h2 className="mt-3 font-semibold">No settings access</h2>
                <p className="mt-1 text-sm text-muted-foreground">Your role does not have permission to manage any settings.</p>
              </div>
            </CardContent>
          </Card>
        )}
