...
type AuthUser = { name?: string; branch_name?: string; branchId?: number | null }

export function Sidebar() {
  const [location, navigate] = useLocation(); const [open, setOpen] = useState(false); const [loggingOut, setLoggingOut] = useState(false); const [permissions, setPermissions] = useState<string[]>([]); const [permissionsLoaded, setPermissionsLoaded] = useState(false); const [user, setUser] = useState<AuthUser | null>(null)
  useEffect(() => { let cancelled = false; fetch("/api/auth/me", { headers: { Accept: "application/json" }, credentials: "same-origin", cache: "no-store" }).then(async (response) => { if (!response.ok) return null; const data = await response.json() as { user?: AuthUser; permissions?: Array<{ key?: string }> }; return { user: data.user ?? null, permissions: Array.isArray(data.permissions) ? data.permissions.map((p) => String(p.key ?? "")).filter(Boolean) : [] }; }).then((data) => { if (!cancelled) { setUser(data?.user ?? null); setPermissions(data?.permissions ?? []); setPermissionsLoaded(true) } }).catch(() => { if (!cancelled) setPermissionsLoaded(true) }); return () => { cancelled = true } }, [])
