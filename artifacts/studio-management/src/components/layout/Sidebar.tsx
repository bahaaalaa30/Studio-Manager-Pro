...
export function Sidebar() {
  const [location, navigate] = useLocation(); const [open, setOpen] = useState(false); const [loggingOut, setLoggingOut] = useState(false)
  const handleLogout = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new Event("smp-logout"))
      setOpen(false)
      navigate("/login")
    } catch {
      setLoggingOut(false)
      window.dispatchEvent(new CustomEvent("smp-auth-error", { detail: "Unable to log out. Please try again." }))
    }
  }
  const navBlock = <>
