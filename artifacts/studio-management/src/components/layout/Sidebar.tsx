...
import { Camera, MonitorPlay, Printer, PackageCheck, UserPlus, Search, Archive, LogOut, Settings, Menu, X, BarChart3 } from "lucide-react"
...
const navItems = [
  ...
  { icon: PackageCheck, label: "Delivery", href: "/delivery" }, { icon: Archive, label: "Archive", href: "/archive" },
  { icon: Search, label: "Customer Track", href: "/track" }, { icon: BarChart3, label: "Orders Analytics", href: "/analytics" },
]

export function Sidebar() {
  const [location] = useLocation(); const [open, setOpen] = useState(false)
  const navBlock = <>
    <div className="flex-1 py-6 px-3 flex flex-col gap-1 overflow-y-auto"><div className="text-xs font-semibold text-sidebar-foreground/50 uppercase tracking-wider mb-2 px-3">Stations</div>{navItems.map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors", location === item.href || (location === "/" && item.href === "/reception") ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/80 hover:text-sidebar-foreground hover:bg-sidebar-accent/50")}><item.icon className="w-4 h-4 shrink-0" />{item.label}</Link>)}</div>
    <div className="p-4 border-t border-sidebar-border shrink-0"><Link href="/settings" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm text-sidebar-foreground/80 hover:text-sidebar-foreground cursor-pointer rounded-md hover:bg-sidebar-accent/50 transition-colors"><Settings className="w-4 h-4" />Settings</Link><button type="button" className="w-full flex items-center gap-3 px-3 py-2 text-sm text-sidebar-foreground/80 hover:text-sidebar-foreground cursor-pointer rounded-md hover:bg-sidebar-accent/50 transition-colors"><LogOut className="w-4 h-4" />Logout</button></div>
  </>
